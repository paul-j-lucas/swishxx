...
token_stream& operator>>( token_stream &ts, token &t ) {
  ...
  while ( ts.get( c ) ) {
    ...
    if ( is_word_char( c ) ) {
      ...
      if ( t.len_ < Word_Hard_Max_Size ) { // continue same word
        t.buf_[ t.len_++ ] = c;
        continue;
      }
    ...
    switch ( c ) {
      case '(': t.type_ = token::tt_lparen; return ts;
      case ')': t.type_ = token::tt_rparen; return ts;
      case '=': t.type_ = token::tt_equal ; return ts;
    } // switch
  } // while
  ...
  if ( in_word ) {
    ...
    //
    // Check to see if the word is an operator.  For only 4 comparisons, linear
