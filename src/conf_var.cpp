...
//*****************************************************************************
{
    ...
    if ( !conf_file ) {
        if ( !::strcmp( file_name, ConfigFile_Default ) ) {
            //
            // The configuration file couldn't be opened; however, the file
            // name is the default, so assume that none is being used and
            // simply return.
            //
            return;
        }
    ...
    while ( c != conf_file.end() ) {
        ...
        ++line_no;
        //
        // See if the line is entirely whitespace optionally followed by a
        // comment starting with '#': if so, skip it.  If we don't end up
        // skipping it, leading whitespace will have been skipped.
        //
        for ( ; c != nl; ++c ) {
            if ( is_space( *c ) )
                continue;
        ...
        if ( c != nl ) {
            //
            // The line has something on it worth parsing further: copy it
            // (less leading and trailing whitespace) to a modifyable buffer
            // and null-terminate it to make that task easier.
            //
            char buf[ 256 ];
            ptrdiff_t len = nl - c;
            ::strncpy( buf, c, len );
