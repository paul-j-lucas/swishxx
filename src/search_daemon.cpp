 ...
 * > bind, and listen, the call to bind fails because the listening server is
 * > trying to bind a port that is part of an existing connection (the one
 * > being handled by the previously spawned child).  But if the server sets
 * > the SO_REUSEADDR socket option between calls to socket and bind, the
...
/**
 * Detach from the terminal.  From [Stevens 1993], p. 417, "Coding Rules":
 *
 * > The first thing to do is call fork and have the parent exit.  This does
 * > several things.  First, if the daemon was started as a simple shell
 * > command, having the parent terminate makes the shell think that the
 * > command is done.  Second, the child inherits the process group ID of the
 * > parent but gets a new process ID, so we're guaranteed that the child is
...
/**
 * Handles a recently accepted socket file descriptor.  If the accept(2) went
 * OK, try to queue the request.  If that doesn't work (because all the request
 * threads are busy and the number of threads in the thread pool has been maxed
 * out, reset the TCP connection effectively telling the client to "go away and
 * try again later."
 *
 ...
 *
 * @return Returns the associated Unix file descriptor.
 */
 ...
 *
 * @param Returns the associated Unix file descriptor.
 */
...
/**
 * Sets the disposition for various signals.
 */
 ...
 * create a socket, detach from the terminal, record our PID, change to the
 * root directory, and finally service requests.  This function never returns.
 *
 * See also:
 *    W. Richard Stevens.  "Advanced Programming in the Unix Environment,"
 *    Addison-Wesley, Reading, MA, 1993.
...
void become_daemon() {
  ...
  if ( !launchd_cooperation ) {
    ...
    //
    // Max-out the amount of CPU time we can run since we will be a daemon and
  ...
  if ( !launchd_cooperation ) {
#endif /* __APPLE__ */
    //
    // If we're root and we've been requested to change our process user/group,
    // do so.
    //
    if ( !group.change_to_gid() ) {
      error() << "can't change group to \"" << group << "\": " << error_string;
      ::exit( Exit_No_Group );
    }
    if ( !user.change_to_uid() ) {
      error() << "can't change user to \"" << user << "\": " << error_string;
      ::exit( Exit_No_Group );
    }
    ...
    //    session leader of a new session, (2) becomes the process group leader
    //    of a new process group, and (3) has no controlling terminal.
    //
    ::setsid();
    ...
    // Ibid.:
    //
    //    Change the current working directory to the root directory.  The
    //    current working directory inherited from the parent could be on a
    //    mounted filesystem.  Since daemons normally exist until the system is
    //    rebooted, if the daemon stays on a mounted filesystem, that
    //    filesystem can not be unmounted.
    //
    if ( ::chdir( "/" ) == -1 ) {
 ...
 * using the \c SO_LINGER socket option.  From [Stevens 1998], p. 187,
 * "SO_LINGER Socket Option":
 *
 * > If l_onoff is nonzero and l_linger is 0, TCP aborts the connection when it
 * > is closed.  That is, TCP discards any data still remaining in the socket
 * > send buffer and sends an RST to the peer, not the normal four-packet
