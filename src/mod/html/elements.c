...
//*****************************************************************************
{
    ...
    // enum so that we can have a simple array of strings below where each
    // "row" has varying length.  It's kind of a hack, but it works.
    //
    ...
    //
    // Elements that have forbidden end tags (obviously) have no tags listed
    // for them in the rows below.
    //
    ...
    // Yes, it's easy, given a start tag, to know what it's end tag is (the
    // same tag, but with a leading '/'); however, the code in parse_html_tag()
    // in mod_html.c is made simpler by explicitly giving the end tag here
    // rather than having to construct a temporary string prepending a '/' to
    // the tag.
    //
    ...
    for ( register char const *const *p = end_tag_table; *p; ++p ) {
        ...
        switch ( v ) {
            ...
            case element::et_optional:
                //
                // Elements that have optional end tags have a null-terminated
            ...
            case element::et_required:
                //
                // Elements that have required and tags have a single closing
                // tag.
                //
                e.close_tags.insert( *++p );
                break;
