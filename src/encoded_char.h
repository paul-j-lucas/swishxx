...
{
    //
    // Remember the current position to allow the decoders to advance through
    ...
    // practice, this particular case should never happen since UTF-7 should be
    // used instead; but you get the idea.)
    //
    // However, handling both an encoding and such a charset simultaneously is
    // a real pain because both can use multiple characters to decode a single
    ...
    // EITHER an encoding OR a non-ASCII/ISO-8859-1 character set, but not
    // both.  If it does, the encoding takes precedence.
    //
    if ( encoding_ )
...
//*****************************************************************************
{
    ...
    if ( decoded_ ) {
        ...
        // we know the delta.  However, since we're about to increment the
        // position to the next character, that character will no longer have
        // been decoded, so set decoded_ to false.
        //
        decoded_ = false;
    } else {
        //
        // The character at the current position has not previously been
        // decoded so we don't know the delta: call decode() to calculate the
        // delta only.  We can't set decoded_ to true since we're about to
        // increment the position to the next character and that character
        // hasn't been decoded.
        //
        decode();
    }
...
// DESCRIPTION
//
//      Returns a pointer to a string converted to lower case taking the
