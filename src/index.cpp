...
//                  this was found to be a good number: higher, the results
//                  don't get better; lower, the results get more striated.
...
//*****************************************************************************
{
    ...
    if ( incremental ) {
        ...
        //
        // In the case where several files (and no directories) are indexed,
        // there would be no directory; however, every file must be in a
        // directory, so add the directory "." here and now to the list of
        // directories.
        //
        check_add_directory( "." );
    }
    ...
    if ( partial_index_file_names.empty() ) {
        ...
        if ( words.size() ) {
            //
            // Since we created partial indicies, write any remaining words to
            // their own partial index so the merge code doesn't have a special
            // case.
            //
            write_partial_index();
        }
        ...
        for ( j = i + 1; j < partial_index_file_names.size(); ++j )
            if ( word[ j ] != words[ j ].end() )
                if ( !::strcmp( *word[ j ], *word[ i ] ) ) {
                    //
                    // The two words are the same: add the second word's file
                    // count to that of the first.
                    //
                    --num_unique_words;
