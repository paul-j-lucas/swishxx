...
run_test_file() {
  TEST_FILE="$1"
  IFS='|' read COMMAND CONF OPTIONS ARGS EXPECTED_EXIT INPUT_FILE < $TEST_FILE
  ...
  if [ "x$INPUT_FILE" = "x" ]; then
      ...
      echo $COMMAND $CHDIR $CONF $OPTIONS $ARGS "< $INPUT_FILE > $OUTPUT 2> $LOG_FILE" > $LOG_FILE
      $COMMAND $CHDIR $CONF $OPTIONS $ARGS < $INPUT_FILE > $OUTPUT 2>> $LOG_FILE
  fi
